...
import './likelihoodGraph.scss';
import { useRef } from 'react';

function LikelihoodGraph() {
    ...
    const fetchData = async () => {
            ...
            if (d.published){
                ...
                data.map((present)=>{
                    if (present.published.getTime() === d.published.getTime()){
                        found = true;
                        if (d.likelihood){
                            present.likelihood += d.likelihood;
                        }
                    }
                })
                if (!found && d.likelihood){
                    data.push({published:d.published , likelihood:d.likelihood});
                }
            }
        })
        ...
        const yMaxValue = d3.max(data, d => d.likelihood);

        d3.select('#likelihoodGraph')
            .select('svg')
            .remove()

        // create chart area
        const svg = d3
            .select('#likelihoodGraph')
            .append('svg')
    ...
    useEffect(()=>{
        if (likelihoodRef.current){
            setWidth(likelihoodRef.current.offsetWidth/1.2);
            fetchData();
        }
  ...
  return (
  	<div className='likelihoodCard' ref={likelihoodRef}>
        <div id='likelihoodGraph'/>
        <h1>Likelihood Variation</h1>
        <p>This show how likelihood of articles has varied across time. </p>
        <p>If 2 or more article have been published on the same day, they have their likelihood added.</p>
    </div>
